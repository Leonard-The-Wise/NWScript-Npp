...
### Menu option - "Compiler settings":
   ...
   - Also, to compile a script, you **MUST** have at least the `nwscript.nss` file present in one of your compilation paths (except output directories). If you set the plugin to a Neverwinter Nights installation path, that will be automatically loaded for you, so you won't have to worry about that.
   - Neverwinter Nights 1 will support any version of the original game and also the Enhanced Edition versions.
...
### Menu option - "User's Preferences":
	...
	** Remarks **
	...
	If you cancel one auto-install attempt (because you refused to run Notepad++ as Administrator), no other attempts will be maded until the next version of Notepad++ - given, of course that the Auto-Install option is still checked when version changes.
...
### Menu option - "About me":
   ...
   - Opens the About box dialog (yeah, really!).
...
Copyright notices for the embbeded version of the [`NWScript Compiler Library`](https://github.com/nwneetools/nwnsc):
  ...
  Readaptations and tweaks for Windows GUI applications usage by: `Leonard-The-Wise`
