   ...
   const basic_char_set<charT, traits>& char_set, std::integral_constant<bool, true>*)
{
   ...
   while(first != last)
   {
      ...
      else
      {
         ...
         std::memset(result->_map + static_cast<unsigned char>(c1), true, static_cast<size_t>(static_cast<unsigned char>(1u)) + static_cast<unsigned char>(static_cast<unsigned char>(c2) - static_cast<unsigned char>(c1)));
      }
   }
   //
   // and now the classes:
